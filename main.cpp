...
void sound_alarm(const std::string&);
double eye_aspect_ratio(const std::vector<dlib::point>&);

int main(void)
{
	// eye aspect ratio thershold to detect eye blink. 
	const double EYE_ASPECT_RATIO_THRESHOLD = 0.09;
	// # of consecutive frames the eye should be below the aspect ratio threshold to sound alarm
	...
	std::string shapePredictorDir = R"(utils\shape_predictor_68_face_landmarks.dat)";
	...
	while (cv::waitKey(30) != 27) // 27 is the ascii code for ESC key. 
	{
		...
		for (const auto& face : faces)
		{
			...
			double averageAspectRatio = static_cast<double>((leftAspectRatio + rightAspectRatio) / 2);
			...
			drawContours(frame, std::vector<std::vector<cv::Point>>{leftHull}, -1, color, 1);
			drawContours(frame, std::vector<std::vector<cv::Point>>{rightHull}, -1, color, 1);
			...
			if (averageAspectRatio < EYE_ASPECT_RATIO_THRESHOLD)
			{
				...
				if (counter >= EYE_ASPECT_RATIO_CONSECUTIVE_FRAMES)
				{
					...
					if (alarm_on)
					{
						cv::putText(frame, "DROWSINESS ALERT!", cv::Point(10, 30),
